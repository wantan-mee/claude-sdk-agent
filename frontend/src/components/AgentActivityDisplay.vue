<template>
  <div v-if="isStreaming" class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
    ...
    <!-- Current Thinking - PROMINENT DISPLAY for Deep Reasoning -->
    <div v-if="currentThinking" class="mb-3 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-4 border-2 border-purple-300 shadow-sm">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="text-sm font-bold text-purple-700">🧠 Deep Reasoning</span>
        <span class="text-xs text-purple-600 font-medium">(Extended Thinking Active)</span>
      </div>
      <div class="bg-white rounded-lg p-3 border border-purple-200 max-h-64 overflow-y-auto">
        <pre class="text-sm text-gray-800 whitespace-pre-wrap font-mono leading-relaxed">{{ currentThinking }}</pre>
      </div>
      <div class="mt-2 text-xs text-purple-600 italic">
        💡 The agent is analyzing step-by-step before responding
      </div>
    </div>
